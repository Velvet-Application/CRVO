...
type Session={ok:boolean;role:"admin"|"user";must_change_password:boolean;access_profile:"admin"|"service_manager"|"team_manager"|"custom"|"transphere";page_permissions:string[];productivity_scopes:string[];team_scopes:string[];can_manage_bonus_workflow:boolean;};
...
function adminOnlyPath(path:string){return path.startsWith("/animation-mensuelle/payplan")||path.startsWith("/animation-mensuelle/acces")||path.startsWith("/api/payplan")||path.startsWith("/capacitaire")||path.startsWith("/api/capacity-simulator")||path.startsWith("/api/capacity-simple")||path.startsWith("/developpement")||path.startsWith("/api/development")||path.startsWith("/api/transphere/import-book")||path==="/expertise-mobile"||path==="/atelier"||path==="/direction"||path.startsWith("/api/kiosk/atelier")||path.startsWith("/api/kiosk/direction");}
function transphereOnlyAllowed(path:string){return path.startsWith("/transphere")||path.startsWith("/api/transphere/dashboard")||path.startsWith("/api/transphere/animation");}
...
export async function proxy(request:NextRequest){
  ...
  try{
    ...
    if(session.access_profile==="transphere"&&!transphereOnlyAllowed(path)){if(path.startsWith("/api/"))return apiUnauthorized(403,"Compte limité à l'environnement Transphère.");return NextResponse.redirect(new URL("/transphere",request.url));}
