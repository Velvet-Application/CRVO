...
function firstAllowed(session:Session){if(has(session,"reporting")||has(session,"book"))return "/";if(has(session,"worktime"))return "/temps-travail";if(has(session,"settings"))return "/?nav=objectives";if(has(session,"data_rh"))return "/animation-centre/rh";if(has(session,"productivity"))return "/performance/productivite";if(has(session,"monthly_animation"))return "/animation-mensuelle";if(has(session,"cockpit"))return "/cockpit-v2";if(has(session,"bodyshop"))return "/cockpit-v2/carrosserie";if(has(session,"client_dashboard"))return "/dashboard-client";if(has(session,"intelligence"))return "/intelligence";if(has(session,"transphere"))return "/transphere";return "/account";}
...
export async function proxy(request:NextRequest){
  ...
  try{
    ...
    if((session.access_profile==="transphere"||session.access_profile==="transphere_manager")&&!transphereRestrictedAllowed(path)){if(path.startsWith("/api/"))return apiUnauthorized(403,"Compte limité à l'environnement Transphère.");return NextResponse.redirect(new URL(session.access_profile==="transphere_manager"?"/transphere":"/transphere",request.url));}
