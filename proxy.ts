...
function firstAllowed(session:Session){if(has(session,"reporting")||has(session,"book"))return "/";if(has(session,"settings"))return "/?nav=objectives";if(has(session,"data_rh"))return "/data-rh";if(has(session,"productivity"))return "/performance/productivite";if(has(session,"monthly_animation"))return "/animation-mensuelle";if(has(session,"cockpit"))return "/cockpit-v2";if(has(session,"bodyshop"))return "/cockpit-v2/carrosserie";if(has(session,"client_dashboard"))return "/dashboard-client";if(has(session,"intelligence"))return "/intelligence";return "/account";}
function requiredPermission(path:string):string|null{if(path==="/performance/productivite"||path.startsWith("/api/productivity")||path.startsWith("/api/staff/suggestions"))return "productivity";if(path.startsWith("/animation-mensuelle")||path.startsWith("/api/monthly-animation"))return "monthly_animation";if(path.startsWith("/data-rh")||path.startsWith("/api/data-import")||path.startsWith("/api/staff/directory")||path.startsWith("/api/staff/competencies"))return "data_rh";if(path.startsWith("/cockpit-v2/carrosserie")||path.startsWith("/api/bodyshop"))return "bodyshop";if(path.startsWith("/cockpit-v2")||path.startsWith("/api/cockpit-v2"))return "cockpit";if(path.startsWith("/dashboard-client")||path.startsWith("/clients")||path.startsWith("/api/client-dashboard")||path.startsWith("/api/clients"))return "client_dashboard";if(path.startsWith("/intelligence")||path.startsWith("/api/intelligence"))return "intelligence";if(path.startsWith("/book")||path.startsWith("/api/import-book"))return "book";return null;}
function adminOnlyPath(path:string){return path.startsWith("/animation-mensuelle/payplan")||path.startsWith("/animation-mensuelle/acces")||path.startsWith("/api/payplan")||path.startsWith("/capacitaire")||path.startsWith("/api/capacity-simulator")||path==="/atelier"||path==="/direction"||path.startsWith("/api/kiosk/");}

export async function proxy(request:NextRequest){
  const path=request.nextUrl.pathname;if(isStatic(path))return NextResponse.next();if(path==="/api/health")return NextResponse.next();
