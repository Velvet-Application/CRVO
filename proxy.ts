...
function adminOnlyPath(path:string){return path.startsWith("/animation-mensuelle/payplan")||path.startsWith("/animation-mensuelle/acces")||path.startsWith("/api/payplan")||path.startsWith("/capacitaire")||path.startsWith("/api/capacity-simulator")||path.startsWith("/api/capacity-simple")||path.startsWith("/developpement")||path.startsWith("/api/development")||path==="/expertise-mobile"||path==="/atelier"||path.startsWith("/api/kiosk/atelier");}
...
export async function proxy(request:NextRequest){
  ...
  const publicDirection=path==="/direction"||path.startsWith("/api/kiosk/direction");
  if(isStatic(path)||path==="/api/health"||publicClientPortal||publicDirection)return NextResponse.next();
