...
function adminOnlyPath(path:string){return path.startsWith("/animation-mensuelle/payplan")||path.startsWith("/animation-mensuelle/acces")||path.startsWith("/api/payplan")||path.startsWith("/capacitaire")||path.startsWith("/api/capacity-simulator")||path.startsWith("/api/capacity-simple");}

export async function proxy(request:NextRequest){
  const path=request.nextUrl.pathname;
  if(isStatic(path)||path==="/api/health"||isPublicKioskPath(path))return NextResponse.next();
