...
type Payload={month:string;availableMonths:string[];allowedSectors?:string[];period?:Period;sourceStatus?:{presence?:boolean;billedTime?:boolean;comparable?:boolean};totals:{boughtHours:number;soldHours:number;productivity:number|null};sectors:Row[];teams:Row[];collaborators:Row[];error?:string};
...
export default function ProductivitePage(){
  ...
  const filterLabel=useMemo(()=>{const parts:string[]=[];if(sector!=="all")parts.push(sectorOptions.find(([key])=>key===sector)?.[1]??sector);if(team!=="all")parts.push(teamLabel(team));if(collaborator!=="all")parts.push(collaborator);return parts.length?parts.join(" · "):"Tous les périmètres autorisés";},[sector,team,collaborator,sectorOptions]);
  ...
  return <main className={styles.page}>
    <header className={styles.hero}><div><a href="/" className={styles.back}>← PERFORMANCE</a><span>PERFORMANCE MENSUELLE</span><h1>Productivité</h1><p>Heures vendues ÷ heures achetées, toujours sur exactement la même période. Si la période du fichier facturé n'est pas certifiée, le KPI est neutralisé au lieu d'afficher un rendement potentiellement faux.</p></div><div className={styles.filters}>
    ...
    {data&&!loading&&<div className={`${styles.periodStrip} ${periodValid?styles.periodStripOk:styles.periodStripBlocked}`}><div><span>{periodValid?"PÉRIODE COMPARÉE":"PRODUCTIVITÉ SUSPENDUE"}</span><strong>{periodValid?`${dateLabel(data.period?.start)} → ${dateLabel(data.period?.end)}`:"Période des heures vendues à confirmer"}</strong><small>{periodValid?"Les heures achetées sont recalculées sur le même intervalle que les heures vendues.":data.period?.reason||"Aucune productivité ne sera affichée tant que les deux sources ne couvrent pas le même intervalle."}</small></div>{!periodValid&&<a href="/data-rh">CERTIFIER LA PÉRIODE →</a>}</div>}
    <div className={styles.filterSummary}><span>PÉRIMÈTRE AFFICHÉ</span><strong>{filterLabel}</strong></div>
    <section className={styles.kpis}><article><span>HEURES ACHETÉES</span><strong>{loading?"…":`${hours(filteredTotals.boughtHours)} h`}</strong><small>{periodValid?`Présence du ${dateLabel(data?.period?.start)} au ${dateLabel(data?.period?.end)}`:"Valeur de contrôle · période non certifiée"}</small></article><article><span>HEURES VENDUES</span><strong>{loading?"…":filteredTotals.individualBlocked?"ÉQUIPE":`${hours(filteredTotals.soldHours)} h`}</strong><small>{filteredTotals.individualBlocked?"Fixline : mesure collective":periodValid?"Même période que les heures achetées":"Import disponible · ratio bloqué"}</small></article><article><span>PRODUCTIVITÉ</span><strong className={periodValid?tone(filteredTotals.productivity):styles.blocked}>{loading?"…":productivityText}</strong><small>{periodValid?"Vendu / acheté · même période":"Fail closed · aucun ratio trompeur"}</small></article></section>
    ...
    <section className={styles.panel}><div className={styles.panelHead}><div><span>LECTURE</span><h2>{view==="sector"?"Par secteur":view==="team"?"Par équipe":"Par collaborateur"}</h2></div><div className={styles.tabs}><button className={view==="sector"?styles.active:""} onClick={()=>setView("sector")}>Secteurs</button><button className={view==="team"?styles.active:""} onClick={()=>setView("team")}>Équipes</button><button className={view==="person"?styles.active:""} onClick={()=>setView("person")}>Collaborateurs</button></div></div>
      <div className={styles.notice}><b>RÈGLE DE CALCUL</b><span>Les heures vendues et les heures achetées doivent couvrir le même intervalle. Les imports Temps pointé facturé sont désormais rattachés à une date d'arrêté certifiée. FIXLINE reste mesuré au niveau de l'équipe, jamais artificiellement au collaborateur.</span></div>
