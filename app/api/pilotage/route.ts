...
type Workload = { registration: string; work_order: string | null; client: string | null; vin: string | null; sector_key: string; sector_label: string; status: string | null; primary_activity: string | null; age_days: number | null; remaining_minutes: number | null; estimated_total_minutes: number | null; potential_revenue_total: number | null; potential_labor_revenue: number | null; potential_parts_revenue: number | null; potential_other_revenue: number | null };
...
function sourceMode(source: string) { return /sftp/i.test(source) ? "sftp" : "book"; }
...
export async function GET() {
  ...
  if (cfg) {
    ...
    try {
      ...
      if (invoices.length) invoiceToday = { revenue: invoices.reduce((sum, item) => sum + (numeric(item.revenue_total) ?? 0), 0), invoices: invoices.length, available: true, source: "sql" };
        ...
        const finance = await rest<FinanceRow[]>(cfg.url, cfg.key, `kpi_financial_snapshots?select=metrics&snapshot_at=eq.${snapshot.date}&limit=1`);
        const metrics = finance[0]?.metrics ?? {};
        const revenue = numeric(metrics.revenue_day);
        const invoicesDay = numeric(metrics.invoices_day);
        if (revenue != null) invoiceToday = { revenue, invoices: Math.round(invoicesDay ?? 0), available: true, source: "book" };
      }
  ...
  const plans = snapshot.production.map((production) => {
    const sectorKey = sectorByLabel[production.name];
    if (!sectorKey) return null;
    const label = production.name;
    const actual = production.value;
    ...
    return { sectorKey, label, actual, target, gap, attainment: target > 0 ? Math.round(actual / target * 100) : 0, queue: Math.round(numeric(summary?.work_order_count) ?? candidates.length), workloadReady: Boolean(summary) || candidates.some((item) => numeric(item.remaining_minutes) != null), runMaxMinutes: Number(setting.run_max_minutes || 60), fifoShare: Number(setting.fifo_share || .7), highTimeOld, runPool: Math.round(numeric(summary?.run_pool) ?? computedRunPool), remainingHours: numeric(summary?.remaining_hours) ?? computedRemainingHours, potentialRevenue: numeric(summary?.potential_revenue) ?? computedPotentialRevenue, oldest, recommendation };
  ...
  const workloadSummary = { workOrders: Math.round(numeric(totalSummary?.work_order_count) ?? uniqueOrders.size), remainingHours: numeric(totalSummary?.remaining_hours) ?? workload.reduce((sum, item) => sum + (numeric(item.remaining_minutes) ?? 0), 0) / 60, potentialRevenue: numeric(totalSummary?.potential_revenue) ?? workload.reduce((sum, item) => sum + (numeric(item.potential_revenue_total) ?? 0), 0) };
  const productionMode = sourceMode(snapshot.source);

  return NextResponse.json({ snapshot, dataConnected, workloadSnapshot, productionMode, sources: { sftp: productionMode === "sftp", production: productionMode, workloadSql: summaries.length > 0 || workload.length > 0, workloadTime: summaries.some((item) => numeric(item.remaining_hours) != null) || workload.some((item) => numeric(item.remaining_minutes) != null), invoicesSql: invoiceToday.source === "sql", financeBook: invoiceToday.source === "book" }, invoiceToday, workloadSummary, major, plans, methodology: { fifo: "Les dossiers les plus anciens restent prioritaires.", run: "Une part de dossiers courts est injectée lorsque le FIFO seul met en risque le volume du jour.", defaultFifoShare: .7, defaultRunMaxMinutes: 60 } }, { headers: { "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0" } });
}
