...
const fallbackSnapshot: Snapshot = {
  ...
  over20: 379,
  production: [
    { name: "Expertise", value: 68, tone: "coral" },
    { name: "Mécanique", value: 82, tone: "green" },
    { name: "DSP", value: 31, tone: "cyan" },
    { name: "Carrosserie", value: 14, tone: "red" },
    { name: "Préparation", value: 87, tone: "purple" },
    { name: "Qualité", value: 91, tone: "orange" },
    { name: "Sortie usine", value: 108, tone: "blue" },
  ],
};
...
async function rest<T>(url: string, key: string, path: string): Promise<T> {
  const response = await fetch(`${url}/rest/v1/${path}`, {
    headers: supabaseRestHeaders(key, { Accept: "application/json" }),
    cache: "no-store",
  });
...
function metric(metrics: Record<string, number | string>, key: string) {
  return numeric(metrics[key]) ?? 0;
}

function formatSnapshot(row: SnapshotRow): Snapshot {
  ...
  return {
    ...
    entries: metric(metrics, "entries_vop"),
    exits: metric(metrics, "exits_vop"),
    stock: metric(metrics, "factory_stock"),
    over15: metric(metrics, "stock_over_15d"),
    over20: metric(metrics, "stock_over_20d"),
    production: [
      { name: "Expertise", value: metric(metrics, "production_expertise"), tone: "coral" },
      { name: "Mécanique", value: metric(metrics, "production_mechanics"), tone: "green" },
      { name: "DSP", value: metric(metrics, "production_dsp"), tone: "cyan" },
      { name: "Carrosserie", value: metric(metrics, "production_bodywork"), tone: "red" },
      { name: "Préparation", value: metric(metrics, "production_preparation"), tone: "purple" },
      { name: "Qualité", value: metric(metrics, "production_quality"), tone: "orange" },
      { name: "Sortie usine", value: metric(metrics, "production_factory_exit") || metric(metrics, "exits_vop"), tone: "blue" },
    ],
  };
}

function fallbackObjectives(): Objective[] {
  return Object.entries(productionNames).map(([sectorKey, sectorLabel]) => ({
    sectorKey,
    sectorLabel,
    dailyTarget: fallbackTargets[sectorKey] ?? 0,
    minThreshold: null,
    maxThreshold: null,
  }));
}
...
export async function GET() {
  ...
  if (cfg) {
    try {
      const latestRows = await rest<SnapshotRow[]>(cfg.url, cfg.key, "kpi_dashboard_snapshots?select=snapshot_at,source_name,metrics&order=snapshot_at.desc&limit=1");
      if (latestRows[0]) {
        snapshot = formatSnapshot(latestRows[0]);
        dataConnected = true;
      }
    } catch (error) {
      console.error(JSON.stringify({ event: "pilotage_snapshot_failed", message: error instanceof Error ? error.message : "unknown" }));
    }

    try {
      const month = `${snapshot.date.slice(0, 7)}-01`;
      const objectiveRows = await rest<ObjectiveRow[]>(cfg.url, cfg.key, `kpi_monthly_objectives?select=sector_key,sector_label,daily_target,min_threshold,max_threshold&month=eq.${month}&order=sector_key.asc`);
      if (objectiveRows.length) {
        objectives = objectiveRows.map((row) => ({
          sectorKey: row.sector_key,
          sectorLabel: row.sector_label,
          dailyTarget: numeric(row.daily_target) ?? 0,
          minThreshold: numeric(row.min_threshold),
          maxThreshold: numeric(row.max_threshold),
        }));
      }
    } catch (error) {
      console.error(JSON.stringify({ event: "pilotage_objectives_failed", message: error instanceof Error ? error.message : "unknown" }));
    }
    ...
    try {
      ...
      console.error(JSON.stringify({ event: "pilotage_workload_failed", message: error instanceof Error ? error.message : "unknown" }));
    }
    ...
    try {
      ...
      console.error(JSON.stringify({ event: "pilotage_settings_failed", message: error instanceof Error ? error.message : "unknown" }));
    }
    ...
    try {
      ...
      console.error(JSON.stringify({ event: "pilotage_invoices_failed", message: error instanceof Error ? error.message : "unknown" }));
    }
  }
