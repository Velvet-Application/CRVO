...
function canManageSessions(profile:string,role:string){return role==="admin"||profile==="hr";}
function message(error:unknown,fallback:string){
  ...
  const known:Record<string,string>={
    ...
    training_session_manage_forbidden:"La modification et la suppression des formations programmées sont réservées aux RH et administrateurs.",
    training_session_delete_not_planned:"Seule une formation encore programmée peut être supprimée.",
    training_session_signed_locked:"Cette formation possède déjà des émargements signés et ne peut plus être modifiée ou supprimée.",
    training_session_afest_locked:"Cette formation est liée à un dossier AFEST déjà engagé dans le circuit EDI/OPCO et ne peut plus être modifiée ou supprimée.",
    employee_not_found:"Collaborateur carrosserie introuvable.",
    track_not_found:"Référentiel métier introuvable.",
    invalid_status:"Statut invalide.",
    invalid_priority:"Priorité invalide.",
    plan_not_found:"Plan de formation introuvable.",
    session_not_found:"Session de formation introuvable.",
    title_required:"Le titre de la formation est obligatoire.",
    invalid_period:"La période de formation est invalide.",
    invalid_kind:"Type d'évaluation invalide.",
    invalid_scores:"Les notes de compétences sont invalides.",
    at_least_one_score_required:"Renseigne au moins une compétence avant d'enregistrer l'évaluation.",
    invalid_category:"Type d'observation invalide.",
    content_required:"L'observation ne peut pas être vide."
  };
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(action==="save-session"){
      const isUpdate=Boolean(nullable(body.id));
      if(isUpdate&&!canManageSessions(current.session.access_profile,current.session.role))return json({error:"La modification des formations programmées est réservée aux RH et administrateurs."},403);
    ...
    if(action==="delete-session"){
      if(!canManageSessions(current.session.access_profile,current.session.role))return json({error:"La suppression des formations programmées est réservée aux RH et administrateurs."},403);
      const sessionId=nullable(body.id);if(!sessionId)return json({error:"Session de formation introuvable."},400);
      const result=await authRpc<Record<string,unknown>>("kpi_training_session_delete",{p_session_hash:current.tokenHash,p_session_id:sessionId});return json(result);
    }
