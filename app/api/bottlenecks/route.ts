...
type RpcPayload = { ok?:boolean; today?:string; cutoff?:string; month?:string; frozenRows?:DailyRow[]; liveRows?:LiveRow[]; objectiveRows?:ObjectiveRow[] };
type Sector = {
  key: string;
  label: string;
  color: string;
...
const configs = [
  ...
  { key:"dsp", label:"DSP", color:"#004f9f" },
  { key:"jantes", label:"Jantes", color:"#f5a623" },
  { key:"mecanique", label:"Mécanique", color:"#55b779" },
  { key:"carrosserie", label:"Carrosserie", color:"#009edb" },
...
function buildPayload(frozenRows:DailyRow[],liveRows:LiveRow[],objectiveRows:ObjectiveRow[],today:string,cutoff:string,backend:string){
  if(!liveRows.length&&!frozenRows.length)return null;
  ...
  const liveModified=liveRows.map(r=>r.source_modified_at).filter(Boolean).sort().at(-1)??frozenRows.map(r=>r.source_modified_at).filter(Boolean).sort().at(-1)??null;
  return {
    connected:true,
    latestDate:liveRows[0]?.snapshot_date??frozenRows.at(-1)?.snapshot_date??today,
    operationalLatestDate:liveRows[0]?.snapshot_date??frozenRows.at(-1)?.snapshot_date??today,
    ...
    critical:sectors.filter(sector=>sector.max!=null&&sector.actual>sector.max).length,
    sectors,
  };
}
...
export async function GET(){
  ...
  try{session=await currentSession();}
    ...
    console.error(JSON.stringify({event:"bottlenecks_session_failed",message:error instanceof Error?error.message:"unknown"}));
    return NextResponse.json({connected:false,error:"Connexion aux données CRVO temporairement indisponible."},{status:503,headers:{"Cache-Control":"no-store"}});
  }
  ...
  try{
    const rpc=await authRpc<RpcPayload>("kpi_bottlenecks_get",{p_token_hash:session.tokenHash,p_days:30});
    const payload=buildPayload(rpc.frozenRows??[],rpc.liveRows??[],rpc.objectiveRows??[],rpc.today??today,rpc.cutoff??cutoff,"rpc");
    if(payload)return NextResponse.json(payload,{headers:{"Cache-Control":"no-store"}});
    throw new Error("Aucune photo EtatduParc dans le RPC.");
  }catch(primaryError){
    console.warn(JSON.stringify({event:"bottlenecks_rpc_fallback",message:primaryError instanceof Error?primaryError.message:"unknown"}));
    const db=env();
    if(!db)return NextResponse.json({connected:false,error:"Encours réels indisponibles : connexion base non configurée."},{status:503,headers:{"Cache-Control":"no-store"}});
    try{
      ...
      const payload=buildPayload(frozenRows,liveRows,objectiveRows,today,cutoff,"rest-fallback");
      if(payload)return NextResponse.json(payload,{headers:{"Cache-Control":"no-store"}});
      return NextResponse.json({connected:false,error:"Aucune photo réelle EtatduParc disponible."},{status:503,headers:{"Cache-Control":"no-store"}});
    }catch(fallbackError){
      console.error(JSON.stringify({event:"bottlenecks_real_source_failed",primary:primaryError instanceof Error?primaryError.message:"unknown",fallback:fallbackError instanceof Error?fallbackError.message:"unknown"}));
      return NextResponse.json({connected:false,error:"Encours réels temporairement indisponibles. Le FTP continue d’être supervisé."},{status:503,headers:{"Cache-Control":"no-store"}});
    }
  }
}
