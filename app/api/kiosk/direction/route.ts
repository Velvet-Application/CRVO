...
async function loadDirectionFinance(history: boolean) {
  const response = await fetch(`${CRVO_SUPABASE_URL}/rest/v1/rpc/kpi_kiosk_direction_finance`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ p_history: history }),
    cache: "no-store",
  });
  if (!response.ok) {
    const detail = await response.text().catch(() => "");
    throw new Error(`Finance kiosk ${response.status}${detail ? `: ${detail.slice(0, 300)}` : ""}`);
  }
