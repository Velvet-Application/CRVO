...
export default function NotificationSoundSettings(){
  ...
  async function save(){setBusy(true);setSaved(false);setError("");try{const r=await fetch("/api/notification-sound",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({sound,volume})});const p=await r.json() as Payload;if(!r.ok)throw new Error(p.error||"Enregistrement impossible.");setSaved(true);window.setTimeout(()=>setSaved(false),2400)}catch(e){setError(e instanceof Error?e.message:"Enregistrement impossible.")}finally{setBusy(false)}}
