...
import {armNotificationAudio,playNotificationSound,type NotificationSoundPreferences} from "./lib/notification-sound";
...
type PreferencePayload={preferences?:NotificationSoundPreferences};
const POPUP_KINDS=new Set(["quality_claim_received","quality_claim_message","internal_chat_message"]);
const SOUND_KINDS=new Set(["quality_claim_message","internal_chat_message"]);
const DEFAULT_PREFERENCES:NotificationSoundPreferences={sound:"crystal",volume:.25};

export default function NotificationCenter(){
  const[toast,setToast]=useState<NotificationRow|null>(null);
  const preferences=useRef<NotificationSoundPreferences>(DEFAULT_PREFERENCES);
  const loadPreferences=useCallback(async()=>{
    try{const r=await fetch(`/api/notification-sound?_=${Date.now()}`,{cache:"no-store"});if(!r.ok)return;const p=await r.json() as PreferencePayload;if(p.preferences)preferences.current=p.preferences}catch{}
  },[]);
    ...
    try{
      ...
      if(next){
        ...
        if(!sessionStorage.getItem(key)){
          ...
          if(SOUND_KINDS.has(next.kind)&&Number.isFinite(created)&&age>=0&&age<=90000)void playNotificationSound(preferences.current.sound,preferences.current.volume);
        }
      }
    }catch{}
  },[]);
  useEffect(()=>{
    const disarm=armNotificationAudio();
    const changed=(event:Event)=>{const detail=(event as CustomEvent<NotificationSoundPreferences>).detail;if(detail)preferences.current=detail};
    window.addEventListener("crvo-notification-sound-changed",changed);
    void (async()=>{await loadPreferences();await load()})();
    const refresh=()=>{if(document.visibilityState==="visible")void load()};
    const timer=window.setInterval(refresh,12000);
    document.addEventListener("visibilitychange",refresh);window.addEventListener("focus",refresh);
    return()=>{disarm();window.clearInterval(timer);document.removeEventListener("visibilitychange",refresh);window.removeEventListener("focus",refresh);window.removeEventListener("crvo-notification-sound-changed",changed)};
  ...
  const isMessage=toast.kind==="internal_chat_message"||toast.kind==="quality_claim_message";
  return <div className={`crvo-notification-toast ${toast.severity}`} role="alert" aria-live="polite">
    <div className="crvo-toast-icon">{isMessage?"✉":toast.kind.startsWith("quality_claim")?"RQ":"!"}</div>
