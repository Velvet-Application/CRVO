...
function blankOutlookWindow() {
  ...
  try {
    ...
    tab.document.body.innerHTML = '<div style="text-align:center"><div style="font-size:42px;font-weight:800;font-style:italic">CRVO</div><p style="font-size:14px;color:#607d94">Préparation du brouillon Outlook, de la diffusion et du PDF…</p></div>';
  } catch {}
  return tab;
}
...
export default function DailyAnimationOneClickV2() {
  ...
  async function warmup(force = false) {
    ...
    const task = (async () => {
      setState("warming");
      setError("");
      const response = await fetch(`/api/daily-animation?_=${Date.now()}`, {
      ...
      try {
        const extraResponse = await fetch(`/api/daily-animation-enrichment?date=${encodeURIComponent(payload.reportDate)}&_=${Date.now()}`, {
          cache: "no-store", headers: { "Cache-Control": "no-cache" },
        });
      ...
      throw reason;
    }).finally(() => { warmingRef.current = null; });
  ...
  async function openAnimation() {
    ...
    try {
      ...
      if (currentSummary.outlook?.graphConfigured) {
        ...
        if (!payload.webLink) throw new Error("Brouillon créé mais lien Outlook indisponible.");
        if (outlookTab) outlookTab.location.replace(payload.webLink);
        else window.location.href = payload.webLink;
        setNotice(`Brouillon Outlook complet · ${toCount} destinataires · ${ccCount} CC · objet · signature · PDF joint.`);
        setState("ready");
        return;
      }
  ...
  return <div className={styles.root}>
    ...
    {settingsOpen ? <div className={styles.panel}>
      ...
      <div className={styles.panelActions}>
        ...
        <button type="button" className={styles.primary} onClick={() => setSettingsOpen(false)}>Fermer</button>
      </div>
    </div> : null}
