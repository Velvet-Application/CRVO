...
export default function WorktimePage(){
  ...
  async function setStatus(row:EventRow,action:"close"|"reopen"|"cancel"){if(row.source==="data_rh")return;if(action==="cancel"&&!confirm(`Annuler l'événement de ${row.employeeName} ?`))return;if(action==="close"&&!confirm(`Clôturer l'événement de ${row.employeeName} ? Le terrain ne pourra plus le modifier.`))return;const response=await fetch("/api/worktime",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,id:row.id})});const payload=await response.json().catch(()=>({})) as{error?:string};if(!response.ok){setError(payload.error||"Modification impossible.");return;}setNotice(action==="close"?"Événement clôturé et verrouillé.":action==="reopen"?"Événement réouvert.":"Événement annulé.");await load();}
      ...
      <article className={styles.roster}>
        ...
        <div className={styles.peopleGrid}>
          {displayedPeople.map(person=>{const daily=(eventsByEmployee.get(person.employeeKey)??[]).filter(item=>item.kind==="absence"?inRange(focusDate,item):item.startDate===focusDate);const absence=daily.find(item=>item.kind==="absence");return <div key={person.employeeKey} className={`${styles.personCard} ${absence?styles.personAbsent:""}`}>
            <div className={styles.personTop}><div><strong>{person.name}</strong><span>{person.team?`Équipe ${person.team}`:"Journée"} · {SECTOR_LABEL[person.sector??""]??person.service??"—"}</span></div>{absence?<b className={styles.absenceBadge}>{LABELS[absence.reason]??"Absent"}{absence.durationHours?` · ${num(Number(absence.durationHours),1)} h`:""}</b>:<b className={styles.presentBadge}>PRÉSENT</b>}</div>
