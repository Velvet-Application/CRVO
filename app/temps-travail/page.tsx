...
export default function WorktimePage(){
  ...
  function openEvent(person:Person,nextKind:Kind){const shift=personShift(person);setSelected(person);setKind(nextKind);setStartDate(focusDate);setEndDate(focusDate);setReason(nextKind==="absence"?"paid_leave":"other");setComment("");setEventTime(nextKind==="late"?shift.start:nextKind==="early_departure"?shift.end:"");}
  ...
  async function exportXlsx(){if(!data)return;const XLSX=await import("@e965/xlsx");const rows:Array<Record<string,unknown>>=[];for(const item of historyEvents){let cursor=new Date(`${item.startDate}T12:00:00Z`);const end=new Date(`${item.endDate}T12:00:00Z`);while(cursor<=end){rows.push({Entité:data.entity,Date:cursor.toISOString().slice(0,10),Matricule:data.people.find(p=>p.employeeKey===item.employeeKey)?.matricule??"",Collaborateur:item.employeeName,Équipe:item.team??"",Secteur:SECTOR_LABEL[item.sector??""]??item.sector??"",Service:item.service??"",Événement:KIND_LABEL[item.kind],Motif:LABELS[item.reason]??item.reason,Heure:item.eventTime??"",Justificatif:item.justification==="received"?"Reçu":item.justification==="pending"?"En attente":"Non requis",Statut:item.status==="closed"?"Clôturé":"Ouvert",Commentaire:item.comment??"",Déclaré_par:item.createdBy,Déclaré_le:item.createdAt,Clôturé_par:item.closedBy??"",Clôturé_le:item.closedAt??""});cursor.setUTCDate(cursor.getUTCDate()+1);}}
    const sheet=XLSX.utils.json_to_sheet(rows);sheet["!cols"]=[12,12,14,28,10,18,12,18,32,10,14,12,36,20,20,20,20].map(w=>({wch:w}));const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,sheet,"Temps de travail");
  ...
  function changeEntity(next:"CRVO"|"TRANSPHERE"){setEntity(next);setEventFilter("all");void load(next,from,to);}
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.workspace}>
      <article className={styles.roster}>
        <div className={styles.sectionHead}><div><span>POSTE DU {displayDate(focusDate).toUpperCase()}</span><h2>{displayedPeople.length} {eventFilter==="all"?"collaborateur(s)":"concerné(s)"}</h2></div><small>{eventFilter==="all"?"Déclaration rapide · uniquement votre périmètre":`Filtre actif : ${eventFilter==="pending"?"justificatif en attente":KIND_LABEL[eventFilter as Kind]}`}</small></div>
        <div className={styles.peopleGrid}>
          {displayedPeople.map(person=>{const daily=(eventsByEmployee.get(person.employeeKey)??[]).filter(item=>item.kind==="absence"?inRange(focusDate,item):item.startDate===focusDate);const absence=daily.find(item=>item.kind==="absence");return <div key={person.employeeKey} className={`${styles.personCard} ${absence?styles.personAbsent:""}`}>
            ...
            {daily.filter(item=>item.kind!=="absence").map(item=><div key={item.id} className={`${styles.inlineEvent} ${item.kind==="late"?styles.late:styles.early}`}>{KIND_LABEL[item.kind]} {item.eventTime?`· ${item.eventTime}`:""}</div>)}
      ...
      <article className={styles.history}>
        <div className={styles.sectionHead}><div><span>HISTORIQUE · {displayDate(from)} → {displayDate(to)}</span><h2>Événements déclarés</h2></div><small>{historyEvents.length} événement(s)</small></div>
        <div className={styles.historyList}>{historyEvents.map(row=><div className={styles.historyRow} key={row.id}>
          <div className={`${styles.kindDot} ${row.kind==="absence"?styles.absent:row.kind==="late"?styles.late:styles.early}`}/><div className={styles.historyMain}><strong>{row.employeeName}</strong><span>{KIND_LABEL[row.kind]} · {LABELS[row.reason]??row.reason}</span><small>{row.startDate===row.endDate?displayDate(row.startDate):`${displayDate(row.startDate)} → ${displayDate(row.endDate)}`}{row.eventTime?` · ${row.eventTime}`:""} · {SECTOR_LABEL[row.sector??""]??row.sector??"—"} · déclaré par {row.createdBy}</small>{row.comment&&<p>{row.comment}</p>}</div>
