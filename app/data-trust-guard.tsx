...
type State="checking"|"ok"|"unavailable";
export default function DataTrustGuard(){
  ...
  useEffect(()=>{document.body.classList.toggle("crvo-data-unavailable",state==="unavailable");return()=>document.body.classList.remove("crvo-data-unavailable");},[state]);
  if(state!=="unavailable")return null;
  return <div className="crvo-trust-alert" role="alert"><span>DONNÉES NON CERTIFIÉES</span><strong>Le flux de pilotage ne répond pas.</strong><p>Les indicateurs opérationnels sont neutralisés pour éviter d'afficher une valeur de démonstration ou périmée comme une donnée CODIR. Réessaie après rétablissement de la source.</p><button type="button" onClick={()=>location.reload()}>RÉESSAYER</button><style>{`
    .crvo-data-unavailable .view-page{filter:grayscale(.7);opacity:.20;pointer-events:none;user-select:none}.crvo-data-unavailable .sidebar-bottom{opacity:.35}.crvo-trust-alert{position:fixed;z-index:180;left:calc(250px + 50%);top:150px;transform:translateX(-50%);width:min(620px,calc(100vw - 300px));padding:25px 28px;border:1px solid #efc8c5;border-left:5px solid #eb5b56;border-radius:14px;background:#fff;box-shadow:0 24px 60px rgba(26,58,79,.22);font-family:Exo,Arial,sans-serif;color:#17364d}.crvo-trust-alert span{display:block;color:#c53b39;font-size:9px;font-weight:800;letter-spacing:.13em}.crvo-trust-alert strong{display:block;margin-top:6px;color:#004f9f;font-size:22px;font-weight:800;font-style:italic}.crvo-trust-alert p{margin:10px 0 16px;color:#667f8f;font-size:11px;line-height:1.55}.crvo-trust-alert button{min-height:38px;padding:0 14px;border:0;border-radius:8px;background:#004f9f;color:#fff;font:800 9px Exo,Arial,sans-serif;letter-spacing:.08em}@media(max-width:760px){.crvo-trust-alert{left:50%;top:90px;width:calc(100vw - 28px)}}@media print{.crvo-trust-alert{display:none!important}}
  `}</style></div>;
}
