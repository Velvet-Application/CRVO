...
type StaffMap={id:string;mechanic_name:string;team_code:TeamCode;workcenter:string;active:boolean};
type Payload={
  generatedAt:string;
  ...
  readiness:{teamMapping:number;teamHours:number;clientRows:number};
};
...
export default function CarrosseriePage(){
  ...
  return <main className={styles.page}>
    <header className={styles.hero}>
      <div><a href="/cockpit-v2?section=pilotage" className={styles.back}>← CRVO COCKPIT V2</a><span>FOCUS MÉTIER · CARROSSERIE</span><h1>PERFORMANCE CARROSSERIE</h1><p>Production par moyen, efficacité des équipes, heures vendues / achetées, temps client et simulation de capacité.</p></div>
    ...
    <section className={styles.mainGrid}>
      <article className={styles.panel}>
        ...
        <div className={styles.teamGrid}>{teamSummary.map(team=><div className={styles.teamCard} key={team.team}><header><span>ÉQUIPE</span><strong>{team.team}</strong></header><div><span>Heures vendues</span><b>{team.sold?fmt(team.sold,1):"—"}</b></div><div><span>Heures achetées</span><b>{team.bought?fmt(team.bought,1):"—"}</b></div><div><span>Efficacité</span><b className={team.efficiency!=null&&team.efficiency>=100?styles.good:styles.watch}>{pct(team.efficiency)}</b></div><footer><span>Dossiers facturés</span><b>{team.dossiers||"—"}</b></footer></div>)}</div>
        {!data?.readiness.teamHours&&<div className={styles.dataNote}><strong>Les heures par équipe s’activeront après tes imports directs.</strong><span>Heures vendues = temps pointé sur dossiers facturés. Heures achetées = temps RH des collaborateurs carrosserie mappés en A/B/C.</span></div>}
      </article>
      ...
      <article className={styles.panel}>
        ...
        <div className={styles.simResult}><div><span>Volume / jour simulé</span><strong>{fmt(simulated.total,1)}</strong><small>{simulated.delta>=0?"+":""}{fmt(simulated.delta,1)} vs moyenne</small></div><div><span>Volume / semaine</span><strong>{fmt(simulated.weekly,0)}</strong><small>5 journées opérationnelles</small></div></div>
