...
export default function DataRhPage(){
  return <main className={styles.page}>
    ...
    <section className={styles.grid}>
      ...
      <article><span>2</span><h2>Temps pointé facturé</h2><p>Temps facturé par intervention et par collaborateur. Il alimente les heures vendues et la productivité.</p></article>
    ...
    <DirectFileImport />
    <section className={styles.panel}><div><span>PROCESSUS</span><h2>Contrôle automatique</h2></div><div className={styles.ready}><i/><strong>Fichier → lecture locale → contrôle → blocs → consolidation</strong><p>Les gros fichiers ne sont plus traités en une seule opération serveur. Les doublons sont bloqués par empreinte SHA-256 et un fichier non conforme est refusé avec les colonnes détectées, sans être interprété silencieusement.</p></div></section>
  </main>;
}
