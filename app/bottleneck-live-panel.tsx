...
type Payload = { latestDate:string; operationalLatestDate:string; stale:boolean; source:string; sourceMode:"sftp"|"book"|"embedded"; critical:number; sectors:Sector[] };
  ...
  useEffect(()=>{
    const locate=()=>{
      ...
      if (!old?.parentElement) {
        document.body.classList.remove("crvo-live-bottlenecks");
        setHost(null); return;
      }
      ...
      document.body.classList.add("crvo-live-bottlenecks");
      setHost(root);
    };
  ...
  return createPortal(<section className="live-bottleneck-panel">
    ...
    {data && current && <>
      <div className={data.stale?"live-source stale":"live-source"}>
        <div><span>{data.sourceMode==="sftp"?"SOURCE SFTP":"SOURCE BOOK"}</span><strong>Encours arrêtés au {longDate(data.latestDate)}</strong></div>
        <div><small>Dernière production disponible</small><b>{longDate(data.operationalLatestDate)}</b></div>
        <div><small>Origine</small><b>{data.source}</b></div>
      </div>
      {data.stale && <div className="live-stale-warning"><strong>DONNÉE ENCOURS EN RETARD</strong><span>Les goulots sont arrêtés au {longDate(data.latestDate)} alors que la production est disponible au {longDate(data.operationalLatestDate)}. Les arbitrages goulot doivent attendre la prochaine donnée encours.</span></div>}
      <div className="live-bottleneck-grid">
        <article className="live-main-card">
          <header><div><span>SECTEUR SÉLECTIONNÉ</span><h3>{current.label}</h3></div><div className="live-kpis"><div><span>ENCOURS ACTUEL</span><strong>{current.actual}</strong></div><div><span>ÉVOLUTION PÉRIODE</span><strong className={current.evolution>0?"bad":"good"}>{current.evolution>0?"+":""}{current.evolution}%</strong></div><div><span>JOURS DE STOCK</span><strong>{current.workDays.toLocaleString("fr-FR",{maximumFractionDigits:2})}</strong></div></div></header>
          <Trend sector={current}/>
          <footer><span className="legend-real" style={{background:current.color}}/><b>Encours réel daté</b><i/><span>Seuil maximum configuré</span><em>Dernier point : {dateLabel(data.latestDate)}</em></footer>
        </article>
