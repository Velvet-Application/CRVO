...
type MentionChoice={kind:"role";token:string;title:string;subtitle:string;count:number;icon:string}|{kind:"claim";token:string;title:string;subtitle:string;claim:Claim;icon:string};
...
export default function InternalChat({sessionName}:{sessionName:string}){
  ...
  const mentionChoices=useMemo<MentionChoice[]>(()=>{
    ...
    const claimMatches=mention.fragment.length<2?[]:claims.filter(c=>normalizeText(`${c.claim_number} ${c.registration} ${c.client_name}`).includes(q)).slice(0,6).map<MentionChoice>(c=>({kind:"claim",token:c.claim_number,title:`@${c.claim_number}`,subtitle:`${c.registration} · ${c.client_name}`,claim:c,icon:"RQ"}));
    return[...roles,...claimMatches].slice(0,8);
  },[mention,data?.users,claims]);
  ...
  function chooseMention(choice:MentionChoice){const current=currentMention(message);if(!current)return;const before=message.slice(0,current.start);setMessage(`${before}@${choice.token} `);if(choice.kind==="claim")setSelectedClaim(choice.claim);setEmojiOpen(false)}
    ...
    <div className={styles.shell}>
      ...
      <section className={styles.chatPane}>
        ...
        {detail?<><header className={styles.chatHead}><button className={styles.mobileBack} onClick={()=>{setActive(null);history.replaceState(null,"","/messagerie");setData(d=>d?{...d,detail:null}:d)}}>←</button>{headerPeer?<Avatar id={headerPeer.id} name={detail.title} updated={headerPeer.avatarUpdatedAt}/>:<i>{initials(detail.title)}</i>}<div><strong>{detail.title}</strong><small>{detail.participants.map(p=>p.displayName).join(" · ")}</small></div><span>INTERNE CRVO</span></header>
          ...
          {mention&&<div className={mentions.mentionPanel}><span className={mentions.mentionTitle}>MENTION INTELLIGENTE</span>{mentionChoices.map(choice=><button type="button" className={mentions.mentionOption} key={`${choice.kind}-${choice.token}`} onClick={()=>chooseMention(choice)}><i className={mentions.mentionIcon}>{choice.icon}</i><span><strong>{choice.title}</strong><small>{choice.subtitle}</small></span><em>{choice.kind==="role"?`${choice.count} profil(s)`:"DOSSIER"}</em></button>)}{!mentionChoices.length&&<p className={mentions.emptyMention}>{claimsLoading?"Recherche des dossiers…":"Aucune mention correspondante."}</p>}</div>}
          <div className={styles.composer}><div className={media.composerTools}><button className={media.toolButton} title="Partager une réclamation" onClick={()=>void openClaimPicker()}>＋</button><button className={media.toolButton} title="Ajouter une photo ou un fichier" onClick={()=>fileRef.current?.click()}>📎</button><button className={media.toolButton} title="Ajouter un emoji" onClick={()=>setEmojiOpen(v=>!v)}>☺</button><input ref={fileRef} className={media.hiddenInput} type="file" multiple accept={FILE_ACCEPT} onChange={e=>{addFiles(e.target.files);e.currentTarget.value=""}}/></div><textarea value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();void send()}}} placeholder="Écrire un message…  @superviseur · @chef-equipe · @JF-2026-0002"/><button className={styles.send} disabled={busy||(!message.trim()&&!selectedClaim&&!pendingFiles.length)} onClick={()=>void send()}>➤</button></div>
          <div className={mentions.mentionHint}>Astuce : <b>@superviseur</b> ou <b>@chef-equipe</b> ouvre le groupe concerné. Tapez <b>@ + numéro de dossier</b> pour joindre automatiquement la réclamation.</div>
