...
type Payload={connected:boolean;date:string;mode:"past"|"today"|"future";isWeekend:boolean;hoursPerProductive:number;teams:TeamRow[];sectors:SectorRow[];shifts:ShiftRow[];reference:{windowStart:string;windowEnd:string;invoiceMinDate?:string|null;invoiceMaxDate?:string|null;invoicedVehicles:number;billedImportedAt?:string|null;complete:boolean;method:string;siteAvgBilledHoursPerVehicle10d?:number|null;activityCapacityMethod?:string|null;siteCapacityMethod?:string|null};summary:{nominal:number;present:number;unavailable:number;pendingLeave:number;productiveHours:number;productiveHoursIfPendingApproved:number;siteTheoreticalVehicles:number|null;siteTheoreticalVehiclesIfPendingApproved:number|null;bottleneckSector:string|null;bottleneckSectorIfPendingApproved:string|null;actualFactoryExits:number|null;dashboardExits:number|null;capacityVsActualPct:number|null};actualSource?:string|null;error?:string};
    ...
    <main className={styles.page}>
      ...
      {!loading&&data&&<>
        <section className={styles.contextBar}>
          ...
          <small>Référence RH : {fmt(data.hoursPerProductive,1)} h / productif · les cadences métier priment sur la moyenne facturée lorsqu’elles sont définies.</small>
        </section>
        ...
        <section className={styles.kpis}>
          ...
          <article className={styles.primary}><span>Capacité théorique site</span><strong>{fmt(summary?.siteTheoreticalVehicles)} VO</strong><small>activité obligatoire limitante : {summary?.bottleneckSector??"—"}</small></article>
        ...
        <section className={styles.siteSummary}>
          <div><p className={styles.eyebrow}>SYNTHÈSE SITE</p><h2>{data.mode==="future"?"Projection de capacité":"Capacité théorique vs réalisé"}</h2><p>La capacité site est désormais pilotée par les cadences métier des étapes obligatoires : Expertise, Préparation, Qualité et Photo. DSP et Jantes restent suivis comme capacités dédiées car ils ne concernent pas systématiquement chaque VO. Mécanique et Carrosserie restent sur leur référence historique jusqu’à définition d’une cadence métier.</p></div>
          <div className={styles.resultBox}><span>Capacité site</span><strong>{fmt(summary?.siteTheoreticalVehicles)} VO</strong><small>Activité obligatoire limitante : {summary?.bottleneckSector??"—"}</small>
