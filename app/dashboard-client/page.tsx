...
const summarySectors = [
  ["expertise_count","Expertise"],["chiffrage_count","Chiffrage"],["controle_technique_count","Contrôle technique"],["dsp_count","DSP"],["jantes_count","Jantes"],["mecanique_count","Mécanique"],["carrosserie_count","Carrosserie"],["preparation_count","Préparation"],["qualite_count","Qualité"],["sortie_usine_count","Sortie usine"],
] as const;
...
export default function DashboardClientPage(){
  ...
  async function searchRegistration(event?:React.FormEvent){event?.preventDefault();const q=registration.trim();if(!q)return;setFocusError("");try{setFocus(await get<FocusPayload>(`/api/client-dashboard?registration=${encodeURIComponent(q)}`));}catch(e){setFocus(null);setFocusError(e instanceof Error?e.message:"Dossier introuvable");}}

  useEffect(()=>{void loadList();const t=window.setInterval(()=>void loadList(),300000);return()=>clearInterval(t);},[]);
  useEffect(()=>{if(mode==="clients"&&selected)void loadClient(selected);},[selected,mode]);
  ...
  function exportCsv(){if(!active)return;const movementMap=new Map((bmw?.metrics.movements??[]).map(m=>[String(m.registration??""),m]));const headers=["Client","Immatriculation","OR","VIN","Modèle","Kilométrage","Position actuelle","Ancienneté usine","Ancienneté position","Travaux restant","Dernier mouvement"];
    const rows=vehicles.map(v=>{const m=movementMap.get(String(v.registration??""));return [active.client,v.registration,v.work_order,v.vin,v.model,v.mileage,v.status,v.factory_age_days,v.status_age_days,remaining(v).join(" | "),m?frDateTime(m.lastEventDate,m.lastEventTime):""].map(csvCell).join(";");});
    const blob=new Blob(["\ufeff"+[headers.map(csvCell).join(";"),...rows].join("\n")],{type:"text/csv;charset=utf-8"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`CRVO_Dashboard_${mode==="bmw"?"BMW_France":active.client.replace(/[^a-z0-9]+/gi,"_")}.csv`;a.click();URL.revokeObjectURL(a.href);
  }
  ...
  return <main className={`dc-page ${mode==="bmw"?"dc-bmw-mode":""}`}>
    ...
    {active&&summary&&<>
      ...
      <section className="dc-age-grid"><article><span>0–15 JOURS</span><strong>{n(summary.age_0_15)}</strong><small>{total?Math.round(n(summary.age_0_15)/total*100):0}%</small></article><article><span>16–20 JOURS</span><strong>{n(summary.age_16_20)}</strong><small>{total?Math.round(n(summary.age_16_20)/total*100):0}%</small></article><article className="watch"><span>21–30 JOURS</span><strong>{n(summary.age_21_30)}</strong><small>{total?Math.round(n(summary.age_21_30)/total*100):0}%</small></article><article className="risk"><span>31 JOURS ET +</span><strong>{n(summary.age_31_plus)}</strong><small>{total?Math.round(n(summary.age_31_plus)/total*100):0}%</small></article>{mode==="bmw"&&bmw&&<article className="alert"><span>DOSSIERS URGENTS</span><strong>{bmw.metrics.urgentCount}</strong><small>signal opérationnel</small></article>}</section>

      <section className="dc-section"><div className="dc-section-head"><div><span>CHARGE RESTANTE</span><h3>Travaux restant à réaliser</h3></div><p>Un véhicule peut être compté dans plusieurs activités lorsqu’il lui reste plusieurs passages.</p></div><div className="dc-sector-grid">{summarySectors.map(([key,label])=><article key={key}><span>{label}</span><strong>{n(summary[key])}</strong></article>)}</div></section>
...
function DossierFocus({payload,onClose}:{payload:FocusPayload;onClose:()=>void}){const v=payload.vehicle;const todo=remaining(v);return <section className="dc-focus"><header><div><span>FOCUS DOSSIER</span><h2>{String(v.registration??"—")}</h2><p>{String(v.client??"—")} · OR {String(v.work_order??"—")}</p></div><button onClick={onClose} className="no-print">FERMER</button></header><div className="dc-focus-grid"><article><span>POSITION ACTUELLE</span><strong>{String(v.status??"—")}</strong><small>depuis {age(v.status_age_days)}</small></article><article><span>ANCIENNETÉ USINE</span><strong>{age(v.factory_age_days)}</strong><small>{String(v.model??"")}</small></article><article><span>RESTE À FAIRE</span><strong>{todo.length}</strong><small>{todo.join(" · ")||"Aucun passage identifié"}</small></article><article><span>POINT DE VIGILANCE</span><strong>{String(v.alert??"—")}</strong><small>{/^oui$/i.test(String(v.urgency??""))?"Priorité renforcée":"Suivi normal"}</small></article></div><div className="dc-log"><div className="dc-section-head"><div><span>HISTORIQUE DOSSIER</span><h3>Journal des mouvements</h3></div><p>{payload.history.length} événements disponibles</p></div>{payload.history.length?<ol>{payload.history.map((e,i)=><li key={`${e.event_date}-${e.event_time}-${i}`}><time>{frDateTime(e.event_date,e.event_time)}</time><div><strong>{e.status||"Événement"}</strong><small>{e.work_order?`OR ${e.work_order}`:""}{e.flow?` · ${e.flow}`:""}</small></div></li>)}</ol>:<div className="dc-empty">Aucun événement historique disponible pour ce dossier dans l’historique actuellement chargé.</div>}</div></section>}
