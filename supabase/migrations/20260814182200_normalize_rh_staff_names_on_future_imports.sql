-- Keep the raw encoded RH identity in presence facts for traceability/team parsing,
-- but store a clean collaborator name plus parsed service/team/matricule in the staff dimension.

create or replace function public.kpi_rh_batch_commit_step_admin(p_session_hash text,p_batch_id uuid,p_days integer default 30)
...
begin
  ...
  if v_user is null then raise exception 'Accès administrateur CRVO requis.' using errcode='42501'; end if;
  ...
  select count(*) into v_received from public.kpi_rh_presence_staging where batch_id=p_batch_id;
  ...
  from (
    select distinct work_date from public.kpi_rh_presence_staging
  ...
  if coalesce(array_length(v_dates,1),0)>0 then
    ...
    update public.kpi_rh_import_batches set committed_rows=v_committed,
      metadata=metadata||jsonb_build_object('delivery_channel','browser_chunked','commit_mode','bounded_by_date','last_commit_at',v_now,'last_commit_rows',v_processed)
    where id=p_batch_id;
  else
    v_committed:=v_batch.committed_rows;
  end if;

  select count(*) into v_remaining from public.kpi_rh_presence_staging where batch_id=p_batch_id;
  if v_remaining=0 and v_committed=v_batch.total_rows then
    ...
    where id=p_batch_id;
    return jsonb_build_object('imported',true,'rows',v_batch.total_rows,'committedRows',v_batch.total_rows,'remainingRows',0,'staffSaved',v_staff,'dateRange',jsonb_build_object('min',v_batch.min_date,'max',v_batch.max_date),'filename',v_batch.filename,'headers',v_batch.headers);
  end if;

  return jsonb_build_object('imported',false,'phase','commit','processedRows',v_processed,'committedRows',v_committed,'remainingRows',v_remaining,'rows',v_batch.total_rows,'dateRange',jsonb_build_object('min',v_batch.min_date,'max',v_batch.max_date),'filename',v_batch.filename);
end $$;
