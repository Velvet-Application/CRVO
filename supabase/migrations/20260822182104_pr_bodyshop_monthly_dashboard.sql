...
begin
  ...
  with months as (
    ...
    select mm.month_start,coalesce(c.amount_ht,0) amount_ht,coalesce(q.vop,0) vop,
      case when coalesce(q.vop,0)>0 then round(coalesce(c.amount_ht,0)/q.vop,2) else 0 end per_vop,
