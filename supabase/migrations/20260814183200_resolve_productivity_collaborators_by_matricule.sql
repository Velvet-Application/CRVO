-- Prefer matricule over name matching for billed-time ↔ RH reconciliation.
-- Keep productivity blank when no billed-time source has been imported for the selected month.
...
begin
  ...
  select exists(
    select 1 from public.kpi_billed_time_facts b
    where b.source_name='Direct Temps pointé facturé'
      and coalesce(b.invoice_date,b.work_date)>=v_start
  ...
  with presence0 as (
    ...
    select b.*,
      coalesce(sd.full_name,public.kpi_rh_base_name(b.mechanic_name)) resolved_name,
      coalesce(sd.name_key,public.kpi_normalize_person_name(public.kpi_rh_base_name(b.mechanic_name))) resolved_key
    from public.kpi_billed_time_facts b
    left join public.kpi_rh_staff_dimension sd
      on nullif(btrim(b.matricule),'') is not null and sd.matricule=btrim(b.matricule)
    where coalesce(b.invoice_date,b.work_date)>=v_start
      and coalesce(b.invoice_date,b.work_date)<v_end
      and b.source_name='Direct Temps pointé facturé'
  ), sold0 as (
    select coalesce(b.invoice_date,b.work_date) work_date,
      b.resolved_name mechanic_name,b.resolved_key name_key,
    ...
    select sector_key,max(sector_label) sector_label,workcenter_key,max(workcenter_label) workcenter_label,sum(bought_hours) bought_hours,sum(sold_hours) sold_hours
    from detail group by sector_key,workcenter_key
  ), teams as (
    select sector_key,max(sector_label) sector_label,workcenter_key,max(workcenter_label) workcenter_label,team_code,sum(bought_hours) bought_hours,sum(sold_hours) sold_hours
    from detail group by sector_key,workcenter_key,team_code
  ), available_months as (
    select distinct to_char(date_trunc('month',d)::date,'YYYY-MM') month_key from (
      select work_date d from public.kpi_sql_presence_facts where work_date is not null
      union all
      select coalesce(invoice_date,work_date) from public.kpi_billed_time_facts where coalesce(invoice_date,work_date) is not null
    ) x
  )
  select jsonb_build_object(
    'month',to_char(v_start,'YYYY-MM'),
    'sourceStatus',jsonb_build_object('presence',exists(select 1 from presence),'billedTime',v_has_sold),
    ...
    'totals',jsonb_build_object(
      ...
      'productivity',case when v_has_sold and coalesce((select sum(bought_hours) from detail),0)>0 then round(coalesce((select sum(sold_hours) from detail),0)/nullif((select sum(bought_hours) from detail),0)*100,1) else null end
    ),
    'sectors',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'productivity',case when v_has_sold and bought_hours>0 then round(sold_hours/bought_hours*100,1) else null end
    ...
    'collaborators',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'productivity',case when v_has_sold and individual_available and bought_hours>0 then round(sold_hours/bought_hours*100,1) else null end,
