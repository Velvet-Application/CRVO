...
begin
  ...
  if v_age is null or v_age > 60 then
    ...
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
      'code','ftp_sync_watch',
      'severity','warning',
      'message','La dernière synchronisation FTP dépasse 30 minutes ; les données restent consultables mais ne doivent pas être lues comme du temps réel.',
      'ageMinutes',round(v_age,1)
    ));
  end if;
