create index if not exists kpi_billed_time_facts_source_file_idx on public.kpi_billed_time_facts(source_name,source_file_sha256);
create index if not exists kpi_rh_staff_dimension_matricule_idx on public.kpi_rh_staff_dimension(matricule) where matricule is not null;
...
begin
  ...
  with presence0 as (
    ...
    select matricule,full_name,name_key from (
      select d.matricule,d.full_name,d.name_key,
        row_number() over(partition by d.matricule order by d.active desc nulls last,d.source_updated_at desc nulls last,d.employee_key) rn
      from public.kpi_rh_staff_dimension d
      where nullif(btrim(d.matricule),'') is not null
    ) q where rn=1
  ), billed_resolved as (
    select bg.*,
    ...
    left join staff_unique su on su.matricule=bg.matricule
  ), sold0 as (
