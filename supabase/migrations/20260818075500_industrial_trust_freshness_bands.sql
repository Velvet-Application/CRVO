...
declare
  ...
  v_ftp_age numeric := null;
begin
  v_base := public.kpi_industrial_health_public();
  v_ftp_age := nullif(v_base->'ftp'->>'syncAgeMinutes','')::numeric;
  ...
  where x->>'code' not in (
    'rh_unmapped_staff','park_duplicate_vin',
    'ftp_sync_stale','ftp_sync_watch','ftp_sync_alert','ftp_sync_non_certified'
  );

  -- Freshness contract for operational presentation:
  -- <= 60 min: normal; >60 min: vigilance; >120 min: alert; >180 min: non-certified.
  if v_ftp_age is null then
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
      'code','ftp_sync_non_certified','severity','critical',
      'message','Aucune synchronisation FTP certifiée n’est disponible.',
      'ageMinutes',null
    ));
  elsif v_ftp_age > 180 then
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
      'code','ftp_sync_non_certified','severity','critical',
      'message','Aucune synchronisation FTP réussie depuis plus de 3 heures : données non certifiées.',
      'ageMinutes',round(v_ftp_age,1)
    ));
  elsif v_ftp_age > 120 then
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
      'code','ftp_sync_alert','severity','warning',
      'message','Aucune synchronisation FTP réussie depuis plus de 2 heures. Vérification des sources recommandée avant arbitrage.',
      'ageMinutes',round(v_ftp_age,1)
    ));
  elsif v_ftp_age > 60 then
    v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
      'code','ftp_sync_watch','severity','warning',
      'message','La dernière synchronisation FTP réussie dépasse 1 heure. Les indicateurs restent consultables avec une fraîcheur dégradée.',
      'ageMinutes',round(v_ftp_age,1)
    ));
  end if;
