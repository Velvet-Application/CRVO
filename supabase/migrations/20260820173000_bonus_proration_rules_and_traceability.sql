...
begin
  ...
  -- Never invent new rules on a closed historical month.
  if v_status in ('closed','legacy_closed') then return; end if;
  insert into public.kpi_bonus_workflow_proration_rules(
    workflow_id,reason_code,label,source_time_codes,individual_mode,individual_threshold_days,
    collective_mode,collective_threshold_days,active,source_revision
  )
  select p_workflow_id,reason_code,label,source_time_codes,individual_mode,individual_threshold_days,
         collective_mode,collective_threshold_days,active,revision
...
begin
  -- Reuse the workflow reader as the source of truth for bonus access/scope.
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'label',r.label,
    'sourceTimeCodes',r.source_time_codes,
    'individualMode',r.individual_mode,
    'individualThresholdDays',r.individual_threshold_days,
    'collectiveMode',r.collective_mode,
    'collectiveThresholdDays',r.collective_threshold_days,
    'active',r.active,
    'sourceRevision',r.source_revision,
    'updatedByName',r.updated_by_name,
    'updatedAt',r.updated_at
  ) order by r.label),'[]'::jsonb) into v_rules
  from public.kpi_bonus_workflow_proration_rules r
  where r.workflow_id=p_workflow_id;

  select coalesce(jsonb_agg(x.item order by x.changed_at desc),'[]'::jsonb) into v_audit
  from (
    select a2.changed_at,jsonb_build_object(
      'id',a2.id,'reasonCode',a2.reason_code,'oldRule',a2.old_rule,'newRule',a2.new_rule,
      'changedByName',a2.changed_by_name,'changedAt',a2.changed_at
    ) item
    from public.kpi_bonus_proration_rule_audit a2
    where a2.workflow_id=p_workflow_id
    order by a2.changed_at desc
    limit 50
  ) x;

  -- Events are returned only for employees already visible in the bonus workflow reader.
  -- The normalized Data RH view keeps the original ingestion lineage and prevents re-keying in the UI.
  with allowed as (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',e.id,'employeeKey',e.employee_key,'employeeName',e.employee_name,'kind',e.event_kind,
    'reason',e.reason_code,'startDate',e.start_date,'endDate',e.end_date,'durationHours',e.duration_hours,
    'justification',e.justification_status,'comment',e.comment,'createdBy',e.created_by_name,
    'createdAt',e.created_at,'source',coalesce(e.source_type,'data_rh')
  ...
  return jsonb_build_object(
    ...
    'sources',jsonb_build_object(
      'events','Data RH · kpi_worktime_rh_event_source',
...
begin
  ...
  if p_individual_mode not in ('inherit','ignore','prorate','suppress') or p_collective_mode not in ('inherit','ignore','prorate','suppress') then
    raise exception 'Mode de proratisation invalide.';
  end if;
  if coalesce(p_individual_threshold_days,0)<0 or coalesce(p_individual_threshold_days,0)>31 or coalesce(p_collective_threshold_days,0)<0 or coalesce(p_collective_threshold_days,0)>31 then
    raise exception 'Le seuil doit être compris entre 0 et 31 jours.';
  end if;
  ...
  update public.kpi_bonus_workflow_proration_rules
  set individual_mode=p_individual_mode,
      ...
      updated_at=now()
  where workflow_id=p_workflow_id and reason_code=p_reason_code
  returning to_jsonb(kpi_bonus_workflow_proration_rules.*) into v_new;
