...
with factory_ranked as (
  select p.*,
    row_number() over (
      partition by p.production_date, p.flow
      order by p.source_modified_at desc nulls last, p.created_at desc, p.id desc
    ) as rn
...
ftp_source as (
  select id from public.kpi_data_sources
...
park_batches as (
  ...
  from (
    select b.id, b.snapshot_at,
      row_number() over (
        partition by b.snapshot_at
        order by coalesce(nullif(b.metadata->>'modified_at','')::bigint,0) desc, b.imported_at desc
      ) as rn
...
park as (
  select snapshot_at,
    count(*) filter (
      where rn=1 and metadata->>'type' in ('VOP EFF','VOP EXT')
...
from factory f
left join park p using(snapshot_at);

create or replace view public.kpi_public_dashboard_snapshots as
select snapshot_at, source_name, metrics
from public.kpi_dashboard_snapshots
where source_name <> 'FTP CRVO · Factory-j+1 + EtatduParc'
union all
select snapshot_at, source_name, metrics
from public.kpi_ftp_daily_history;

grant select on public.kpi_ftp_daily_history to anon, authenticated;
grant select on public.kpi_public_dashboard_snapshots to anon, authenticated;
