...
latest_factory_batch as (
  select b.id, b.imported_at, b.metadata
  from public.kpi_import_batches b
  join ftp_source s on s.id = b.source_id
  where b.original_filename in ('Factory-j_1.csv','Factory-j+1.csv')
    and b.metadata->>'factory_production_status' = 'ready'
...
factory as (
  select d.production_date as snapshot_at,
...
latest_park_batch as (
  select b.id, b.snapshot_at, b.imported_at, b.metadata
  from public.kpi_import_batches b
  join ftp_source s on s.id = b.source_id
...
park as (
  select b.snapshot_at,
         max(v.source_modified_at) as park_modified_at,
         count(*) filter (
           where v.metadata->>'type' in ('VOP EFF','VOP EXT')
             and lower(coalesce(v.status,'')) not in (
               'transport à vide','en attente de transport aller','sortie usine',
               'en attente de transport retour','transport retour planifié','transport retour effectué'
             )
...
select f.snapshot_at,
       ...
       jsonb_build_object(
         'entries_vop',coalesce(f.entries_vop,0),'exits_vop',coalesce(f.production_factory_exit,0),
         'factory_stock',coalesce(p.factory_stock,0),'stock_over_15d',coalesce(p.stock_over_15d,0),
         'stock_over_20d',coalesce(p.stock_over_20d,0),'production_expertise',coalesce(f.production_expertise,0),
         'production_mechanics',coalesce(f.production_mechanics,0),'production_dsp',coalesce(f.production_dsp,0),
         'production_bodywork',coalesce(f.production_bodywork,0),'production_preparation',coalesce(f.production_preparation,0),
         'production_quality',coalesce(f.production_quality,0),'production_factory_exit',coalesce(f.production_factory_exit,0)
       ) as metrics,
